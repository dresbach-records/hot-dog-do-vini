...
export const juridicoController = {
  ...
  async listFiscalSales(req, res) {
    try {
      const [rows] = await db.query('SELECT * FROM fiscal_sales_records ORDER BY data_venda DESC');
      res.json({ success: true, data: rows });
    } catch (error) {
      res.status(500).json({ success: false, error: error.message });
    }
  ...
  async createFiscalSale(req, res) {
    const { data_venda, valor_total, descricao, forma_pagamento, nota_fiscal_vinculada } = req.body;
    try {
      const [result] = await db.query(
        'INSERT INTO fiscal_sales_records (data_venda, valor_total, descricao, forma_pagamento, nota_fiscal_vinculada) VALUES (?, ?, ?, ?, ?)',
        [data_venda, valor_total, descricao, forma_pagamento, nota_fiscal_vinculada]
      );
      res.json({ success: true, id: result.insertId });
    } catch (error) {
      res.status(500).json({ success: false, error: error.message });
